    ...
    def add_embeddings_op(self):
        ...
        with tf.variable_scope("words"):
            _word_embeddings = tf.Variable(
                    tf.constant(0.0, shape=[self.nwords, 100]),
                    name="_word_embeddings",
                    dtype=tf.float32,
                    trainable=False)

            self.word_embeddings_placeholder = tf.placeholder(tf.float32, [self.nwords, 100])
        ...
        with tf.variable_scope("entities"):
            ...
            _entity_embeddings = tf.Variable(
                tf.constant(0.0, shape=[self.nentities, 100]),
                name="_entity_embeddings",
                dtype=tf.float32,
                trainable=self.args.train_ent_vecs)

            self.entity_embeddings_placeholder = tf.placeholder(tf.float32, [self.nentities, 100])
    ...
    def add_lstm_score_op(self):
        ...
        with tf.variable_scope("span_emb_ffnn"):
            ...
            if self.args.span_emb_ffnn[0] == 0:
                span_emb_projected = util.projection(self.span_emb, 100, model=self)
            else:
                hidden_layers, hidden_size = self.args.span_emb_ffnn[0], self.args.span_emb_ffnn[1]
                span_emb_projected = util.ffnn(self.span_emb, hidden_layers, hidden_size, 100,
    ...
    def add_local_attention_op(self):
        ...
        with tf.variable_scope("attention"):
            ...
            if self.args.attention_on_lstm and self.args.nn_components.find("lstm") != -1:
                ...
                att_x_w = util.projection(self.context_emb, 100, model=self) # if tf.shape(self.context_emb)[-1] != 300 else self.context_emb
            ...
            if self.args.attention_use_AB:
                att_A = tf.get_variable("att_A", [100])
            ...
            if self.args.attention_use_AB:
                att_B = tf.get_variable("att_B", [100])
                x_c = att_B * x_c
    ...
    def add_global_voting_op(self):
        with tf.variable_scope("global_voting"):
            ...
            if self.args.global_topkfromallspans:
                ...
                all_entities = tf.reshape(self.pure_entity_embeddings, [batch_num, new_size, 100])
                ...
                if self.args.new_all_voters_emb:
                    ...
                    all_voters_emb = tf.reduce_sum(tf.reshape(masked_entity_emb, [batch_size, -1, 100]), axis=1,
                                                   keep_dims=True)  # [b, 1, 300]
                else:
                    all_voters_emb = tf.reduce_sum(tf.reshape(masked_entity_emb, [-1, 100]), axis=0)  # [300]
