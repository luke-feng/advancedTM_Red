...
class VocabularyCounter(object):
    ...
    def __init__(self, lowercase_emb=False):
        ...
        config.base_folder+"data/basic_data/wordEmbeddings/Word2Vec/2014_tudarmstadt_german_5mincount.bin", binary=True)
        ...
        for chunk in self._generator.process(filepath):
            ...
            for left, right, gt in zip(begin_gm, end_gm, ground_truth):
                ...
                if cand_ent is None:
                    gm_misses += 1
                    cand_ent, scores = [], []
                    #print("cand_ent is none")
                    #print("gm not in p_e_m\t\t\t", colored(' '.join(chunk_words[left:right]), 'blue'))
                elif args.calculate_stats and gt not in cand_ent:
                    gt_misses += 1
                    #print("gt not in cand ent", colored(' '.join(chunk_words[left:right]), 'yellow'))
...
def _parse_args():
    ...
    parser.add_argument("--p_e_m_choice", default="wiki",
    ...
    parser.add_argument("--experiment_name", default="hipe",
...
if __name__ == "__main__":
    ...
    if args.create_entity_universe:
        create_entity_universe(gmonly_files=[], allspans_files=['HIPE-data-v1.0-train-de.txt', 'HIPE-data-v1.0-dev-de.txt','HIPE-data-v1.0-test-de.txt'])
    else:
        create_tfrecords()
