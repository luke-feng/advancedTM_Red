...
def load_wiki_name_id_map(lowercase=False, filepath=None):
    ...
    if filepath is None:
        filepath = config.base_folder + "data/basic_data/wiki_name_map.txt"
    disambiguations_ids = load_disambiguations()
    print("load wiki_id_name_map: {}".format(filepath))
    with open(filepath) as fin:
        for line in fin:
            line = line.rstrip()
            try:
...
def custom_p_e_m(cand_ent_num=30, allowed_entities_set=None,
    ...
    with open(config.base_folder + 'data/basic_data/pro_wikipedia_p_e_m.txt') as fin:
...
def get_immediate_files(a_dir):
    return [name for name in os.listdir(a_dir)
            if (os.path.isfile(os.path.join(a_dir, name)) and ".txt" in name)]
